...
include $(CLEAR_VARS)
UFSD_TOOLS := mkexfat mkntfs chkexfat chkntfs chkufsd
LOCAL_MODULE := ufsd
LOCAL_SRC_FILES := ufsd
...
# All chk*fs and mk*fs utilities inside the archive are actually hardlinks to `ufsd' utility.
# In order to make sure that the files are not taking unnecessary space,
# Install the symlinks.
LOCAL_POST_INSTALL_CMD := $(hide) $(foreach t,$(UFSD_TOOLS),ln -sf ${LOCAL_MODULE} $(LOCAL_MODULE_PATH)/$(t);)

include $(BUILD_PREBUILT)

include $(CLEAR_VARS)
LOCAL_MODULE := sysdump
LOCAL_MODULE_TAGS := optional eng
LOCAL_MODULE_CLASS := EXECUTABLES
LOCAL_CERTIFICATE := platform
LOCAL_MODULE_PATH := $(TARGET_OUT)/xbin
LOCAL_SRC_FILES := sysdump
include $(BUILD_PREBUILT)
