	...
	for_each_old_connector_in_state(old_state, connector,
		...
		if ((old_mode != new_mode) || (old_fps != new_fps)) {
			...
			panel_event_notification_trigger(panel_type,
					&notification);
			// add by zte for tpd event send prints begin
			if (is_pre_commit && (old_mode != new_mode) && (new_mode != DRM_PANEL_EVENT_FPS_CHANGE)) {
				pr_info("msm_lcd change power mode event panel_type=%d connector:%s power mode %d->%d\n",
				panel_type, c_conn->name, old_mode, new_mode);
			}
			// add by zte for tpd event send prints end
		}
	}
